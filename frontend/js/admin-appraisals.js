...
// ── Per-type attribute catalog ──────────────────────────────────────
// Each entry: [field_name, display_label, weight, is_auto]
const TYPE_ATTR_MAP = {
  casa: [
...
function _renderPagination() {
  ...
  return `<div class="admin-pagination ap-flex-row-divider">
    ...
    <span  class="ap-body-text">Pág. ${_appraisalPage} de ${_appraisalPages} (${_appraisalTotal} total)</span>
...
function renderDetail(a) {
  ...
  return `
    ...
    <div class="acm-readonly-banner">
      ...
      ${isCompleted ? `<button class="btn btn-primary ap-btn-compact" id="newVersionBtn"  >+ Nueva versión</button>` : ''}
    </div>` : ''}
      ...
      <div  class="ap-flex-wrap-sm">
        ${isReadOnly
          ...
          : `<button class="btn btn-primary" id="saveBtn">Guardar</button>
              ${hasComps && !isCompleted ? `<button class="btn btn-primary ap-surface-accent" id="completarBtn"  >Guardar Valuación</button>` : ''}
              <button class="btn btn-ghost" id="reportBtn">PDF</button>
              <button class="btn btn-ghost" id="exportCsvBtn">CSV</button>
...
async function openAppraisalPanel(id) {
  ...
  try {
    const a = await API.getAppraisal(id);
    _currentAppraisal = a;
    $('apprPanelTitle').textContent = esc(a.titulo || a.solicitante || `Tasación #${a.id}`);
...
async function renderAppraisalPanel(a, body) {
  ...
  body.innerHTML = `
    <div class="appr-panel-section">
      <div class="appr-panel-section-title">Información General</div>
      ...
      <div class="appr-panel-row">
        <span class="appr-panel-label">Dirección</span>
      ...
      <div class="appr-panel-row">
        ...
        <span class="appr-panel-value">${a.superficie_cubierta ? a.superficie_cubierta + ' m²' : '—'}</span>
      </div>
      <div class="appr-panel-row">
        <span class="appr-panel-label">Dorm / Baños</span>
        <span class="appr-panel-value">${a.dormitorios || 0} dorm · ${a.banios || 0} baños</span>
      </div>
      <div class="appr-panel-row">
    ...
    <div class="appr-panel-section">
      <div class="appr-panel-section-title">Acciones Rápidas</div>
      <div class="appr-actions" id="apprActions"></div>
    </div>
    <div class="appr-panel-section">
      <div class="appr-panel-section-title">Línea de Tiempo</div>
    ...
    <div class="appr-panel-section">
      ...
      <div class="appr-comment-input">
        <textarea id="apprCommentInput" placeholder="Escribí un comentario..." rows="1"></textarea>
  ...
  $('apprCommentInput').onkeydown = e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendComment(a.id); } };
  ...
  try {
    ...
    c.innerHTML = items.map(i => `
      <span class="appr-file-chip">
        ...
        ${esc(i.original_name)}
        <button class="appr-file-delete" onclick="deleteFile(${aid}, ${i.id})" title="Eliminar">×</button>
      </span>
    `).join('');
  } catch {
    c.innerHTML = '<div  class="ap-text-muted-base">Error al cargar</div>';
...
async function showAssignAgent(aid) {
  try {
    ...
    const current = _currentAppraisal?.assigned_agent_id;
