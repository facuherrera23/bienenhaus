...
class Tasacion(db.Model):
    ...
    __table_args__ = (
        db.Index('ix_tasaciones_estado_updated', 'estado', 'updated_at'),
    ...
    agent = db.relationship('Agent', backref=db.backref('tasaciones', lazy='dynamic'))
    ...
    comparables = db.relationship('TasacionComparable', backref='tasacion', lazy='selectin',
                                  cascade='all, delete-orphan',
                                  order_by='TasacionComparable.numero')
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        d['comment_count'] = len(self.tas_comments) if hasattr(self, 'tas_comments') else 0
        d['file_count'] = len(self.tas_files) if hasattr(self, 'tas_files') else 0
        d['timeline_count'] = len(self.tas_timeline) if hasattr(self, 'tas_timeline') else 0
        return d
    ...
    def update_from_dict(self, data: dict[str, Any]) -> None:
        for k, v in data.items():
            if k in self.TASACION_EDITABLE:
                setattr(self, k, v)


class TasacionComparable(db.Model):
    __tablename__ = 'tasacion_comparables'
    __table_args__ = (
        db.Index('ix_tasacion_comparables_numero', 'tasacion_id', 'numero'),
        {'extend_existing': True},
    )
    ...
    def to_dict(self) -> dict[str, Any]:
        cols = [c.name for c in self.__table__.columns if c.name != 'tasacion_id']
        d: dict[str, Any] = {}
        for c in cols:
            v = getattr(self, c)
            if isinstance(v, (datetime, date)):
...
class TasacionLog(db.Model):
    __tablename__ = 'tasacion_logs'
    ...
    tasacion = db.relationship('Tasacion', backref=db.backref('logs', cascade='all, delete-orphan'))
...
class TasacionVersion(db.Model):
    __tablename__ = 'tasacion_versions'
    __table_args__ = ({'extend_existing': True},)
    ...
    tasacion = db.relationship('Tasacion', backref=db.backref('versions', cascade='all, delete-orphan',
                                order_by='TasacionVersion.version.desc()'))
    creator  = db.relationship('User', backref=db.backref('tasacion_versions', lazy='dynamic'))
